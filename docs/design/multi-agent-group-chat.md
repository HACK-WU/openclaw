...
| v3.1 | 2026-03-04 | 增加全面合理性审查结论、技术可行性判定、关键冲突修正建议与场景扩展需求补充                                          |
...
| 评估维度   | 结论 | 说明                                          |
| ---------- | ---- | --------------------------------------------- |
| 角色体系   | 通过 | Owner / 助手 / 普通成员边界清晰，权限差异明确 |
| 分发策略   | 通过 | 单播/广播规则可理解，默认行为明确             |
| Agent 协作 | 通过 | `@` 机制 + `group_reply` 工具形成闭环         |
| 状态管理   | 通过 | 群公告、群 Skill、职责提示词均有独立入口      |
| 风险控制   | 通过 | 防循环 + 只读策略 + 历史压缩形成保护网        |

### 13.2 技术可行性评估

| 模块                             | 可行性 | 评估结论                                                |
| -------------------------------- | ------ | ------------------------------------------------------- |
| 后端独立实现 (`src/group-chat/`) | 高     | 以新目录承载功能，最小侵入现有单聊路径，落地可行        |
| 群聊上下文注入                   | 高     | 可复用现有 System Prompt 装配能力，新增群上下文片段即可 |
| 上下文压缩                       | 中高   | 核心算法可复用，需定制多发言者摘要 Prompt               |
| 并行推理                         | 中高   | 后端可并行触发；关键复杂度在流事件管理与写入顺序控制    |
| 并行流前端渲染                   | 中     | 需要从单流状态重构为多流状态容器（`runId` 级别）        |
| Tool 只读策略                    | 中高   | 可复用现有 Tool Policy Pipeline 机制进行拦截            |

### 13.3 当前文档中需要明确的关键冲突点（必须修订）

1. **“UI 优先”与“后台模拟 group.send”表述冲突**
   - 当前文档同时出现“必须先在 UI 展示后再发送后台”与“后台直接模拟 `group.send`”两种路径。
   - **统一建议**：以**后台 `group.send` 为唯一落库入口**，UI 可做“预展示/流式展示”，但最终以后台确认消息为准。

2. **`sender` 中重复携带 `groupId` 的冗余问题**
   - 消息根字段已有 `groupId`，`sender.groupId` 重复。
   - **建议**：保留消息根 `groupId`，`sender` 仅保留身份字段（`type/agentId/agentName`）。

3. **`@agentName` 的歧义**
   - 成员重名会造成路由歧义。
   - **建议**：协议层仅接受 `agentId`，UI 允许按名称输入但发送前必须解析为唯一 `agentId`。

4. **并行流事件的顺序一致性缺口**
   - 仅按完成时间写入会导致跨客户端顺序不一致体验。
   - **建议**：增加 `serverSeq`（单调递增）用于 UI 一致排序。

5. **Agent sender 的安全校验缺口**
   - 若外部可伪造 `sender.type=agent`，会造成冒充。
   - **建议**：`group.send` 对 `sender=agent` 仅允许内部调用路径（tool/run context）并校验该 agent 属于群成员。

### 13.4 核心业务场景覆盖度评估

| 核心场景                    | 覆盖情况 | 备注                             |
| --------------------------- | -------- | -------------------------------- |
| Owner 指定单 Agent 执行任务 | 已覆盖   | 单播 + @ 可支持                  |
| 多 Agent 并行给出观点       | 已覆盖   | 广播模式 + 并行流                |
| Agent 间协作补充            | 已覆盖   | `group_reply` + mentions         |
| 长会话可持续讨论            | 已覆盖   | historyLimit + compaction        |
| 群内策略统一管理            | 已覆盖   | 群公告 + 群 Skill + 职责提示词   |
| 安全约束与边界控制          | 基本覆盖 | 需补强 sender 安全校验与审计能力 |

---

## 14. 场景扩展需求补充（面向更多应用场景）

### 14.1 企业与团队协作场景

1. **审批闸门（Approval Gate）**
   - 对高风险操作（即使是 Owner 触发）增加二次确认节点。
   - 适用：生产变更、对外发送、批量操作。
...
3. **审计日志与回放**
   - 记录谁触发了谁、何时触发、使用了哪些工具、是否命中只读拦截。
   - 适用：合规审计、问题追踪。

### 14.2 生产可用性场景

4. **故障降级模式**
   - 模型超时/失败时自动降级：并行转串行、成员缩减、仅助手响应。
   - 适用：高峰期或模型不稳定时。

5. **成本治理策略**
   - 群级 Token 预算、日限额、广播次数上限。
   - 适用：多群并发的成本可控。

6. **并发与幂等保护**
   - 为 `group.send` 增加 `clientMessageId` 幂等键，避免重试导致重复消息。
   - 适用：弱网、多端并发。

### 14.3 业务编排与知识协作场景

7. **结构化任务卡（Task Card）**
   - 消息可附带任务状态（todo/in_progress/done）与负责人。
   - 适用：研发协作、项目推进群。
...
| 性能       | 广播模式下首 token 延迟、整体完成时长满足既定阈值     |
