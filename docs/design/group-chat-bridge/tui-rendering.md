...
前端网络抖动或用户切换页面后重新打开，WebSocket 连接断开重连。断线期间 CLI 进程仍在后端运行并产生输出，前端终端组件如何恢复？

### 6.2 方案：后端缓冲 + 重连重放

**核心思路**：后端为每个活跃的 PTY 进程维护一个**环形缓冲区**，存储最近的终端输出数据。前端重连后，后端将缓冲区内容一次性推送，前端 xterm.js 重放恢复。

```
正常连接中：
  PTY → onData → Base64 → WebSocket → 前端 xterm.js ✅

断线期间：
  PTY → onData → Base64 → 写入环形缓冲区（WebSocket 不可用）
  前端：连接断开，终端组件暂停

重连后：
  前端发送 reconnect 请求（携带 groupId + agentId）
      ↓
  后端发送缓冲区中所有数据 → 前端 xterm.js 重放
      ↓
  恢复实时推送，后续数据正常传输
```
...
| 参数       | 建议值            | 说明                               |
| ---------- | ----------------- | ---------------------------------- |
| 缓冲区大小 | 1 MB              | 足以存储约 10,000 行终端输出       |
| 存储格式   | Base64 编码块数组 | 与传输格式一致，重放时无需二次编码 |
| 淘汰策略   | 最旧数据先被覆盖  | 环形缓冲区自然淘汰                 |
| 生命周期   | 跟随 PTY 进程     | 进程结束时释放                     |

### 6.4 前端重连流程

1. **检测断线**：WebSocket `onclose` 事件触发
2. **自动重连**：使用指数退避重试（1s → 2s → 4s → 8s → 最大 30s）
3. **重连成功**：
   - 发送 `group.terminalReconnect` 请求（携带 `groupId`、`agentId`）
   - 后端回放缓冲区数据
   - **重建 xterm.js 实例**（清空旧缓冲区，用回放数据重建终端状态）
4. **恢复实时推送**：回放完成后，切换回正常的 `group.terminal` 事件流

### 6.5 xterm.js 实例管理

| 场景                | 处理方式                                 |
| ------------------- | ---------------------------------------- |
| 短暂断线（< 30s）   | 保留 xterm.js 实例，重连后追加缓冲区数据 |
| 长时间断线（> 30s） | 重建 xterm.js 实例，用缓冲区数据完整重放 |
| 页面刷新            | 重建 xterm.js 实例，用缓冲区数据完整重放 |
| CLI 进程已结束      | 显示最后的终端快照（只读状态）           |
