...
- 宽度填充整个消息气泡
- **顶部状态栏（动态）**：
  - 工作状态：`🔧 Claude Code 正在工作...`（带呼吸动画）
  - 空闲状态：`🔧 Claude Code`（静态显示）
- 完成后顶部折叠条："📦 执行完毕（点击展开终端记录）"

### 2.8 动态状态指示器

**核心思路**：复用终端内容变更检测逻辑，根据终端是否有内容更新，动态切换状态提示。

#### 2.8.1 状态定义

| 状态     | 条件                          | UI 展示                                     |
| -------- | ----------------------------- | ------------------------------------------- |
| **工作** | 终端内容正在更新              | `🔧 Claude Code 正在工作...` （带呼吸动画） |
| **空闲** | 终端内容无更新（超过 2-3 秒） | `🔧 Claude Code` （静态显示）               |

#### 2.8.2 实现机制

```typescript
class BridgeTerminal extends LitElement {
  // 状态
  @state() private _isWorking = false;
  private _idleTimer: number | null = null;
  private readonly IDLE_TIMEOUT = 2500; // 2.5 秒无更新视为空闲
    ...
    if (this._isWorking) {
      return html`
        <div class="status-indicator working">
          ...
          <span class="text">${this.agentName} 正在工作...</span>
      ...
      return html`
        <div class="status-indicator idle">
          <span class="icon">🔧</span>
          <span class="text">${this.agentName}</span>
        </div>
      `;
    }
  }
...
| 时间               | 说明                                          |
| ------------------ | --------------------------------------------- |
| 过短（< 1 秒）     | 频繁切换状态，视觉闪烁，用户体验差            |
| 过长（> 5 秒）     | 状态反馈滞后，用户无法准确判断 CLI 是否在工作 |
| **适中（2-3 秒）** | 平衡了响应性和稳定性，给用户清晰的反馈        |
...
启动新的空闲计时器（2.5 秒）
    ↓
    ├── 2.5 秒内有新数据 → 重置计时器（保持工作状态）
    └── 2.5 秒内无新数据 → 设置 _isWorking = false（切换为空闲）
```
...
.status-indicator.working .pulse {
...
1. ✅ **复用现有逻辑**：利用 `onData` 事件监听，无需额外机制
2. ✅ **实时反馈**：用户清楚知道 CLI 是否在活跃工作
3. ✅ **视觉友好**：通过呼吸动画增强工作状态的感知
4. ✅ **自动切换**：无需手动操作，状态自动根据终端活动切换
5. ✅ **性能优化**：仅在状态切换时更新 UI，不会频繁重绘
...
function extractVisibleText(terminal: Terminal): string {
  ...
  // 1. 合并行并规范化（清理终端框架字符、空白行）
  let text = normalizeExtractedTerminalText(lines.join("\n"));

  // 2. 过滤上下文消息（系统上下文、身份角色等）
  text = filterContextBlock(text);

  // 3. 始终调用 stripAnsiEscapes() 进行 ANSI 二次清理
...
提取文本时需要识别并排除后端注入的上下文消息（详见 [CLI Agent 上下文](./cli-agent-context.md)）：
...
1. 完成检测触发后，前端**等待 5 秒**确保终端内容稳定
2. 从 xterm.js buffer 中提取文本
3. 过滤上下文消息、清理 ANSI 序列
4. 通过 WebSocket 将提取的纯文本发回后端
5. 后端接收纯文本（不做任何处理），作为 `group.stream` final 消息广播并写入 transcript

> **详细设计**：见 [终端内容提取到对话的设计](./terminal-content-extraction.md)
