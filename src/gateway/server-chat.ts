...
export function createAgentEventHandler({
  ...
  const emitChatDelta = (
    ...
    if (prevRaw && cleaned.length < prevRaw.length && !cleaned.startsWith(prevRaw)) {
    ...
    chatRunState.lastRawText.set(clientRunId, cleaned);
    ...
    const segments = completed && completed.length > 0 ? [...completed, cleaned] : undefined;
    ...
    // Normal throttling logic (leading-edge with trailing-edge timer)
    const now = Date.now();
